...
export default function AgentOperations({
  ...
  function inspectAgent(agent: AgentView) {
    onOpenDetail({
      ...
      tone: agent.enabled ? (agent.runtime.status === "healthy" ? "mint" : agent.runtime.status === "watch" ? "amber" : "neutral") : "red",
      owner: agent.ownerRoles.join(" · "),
      scope: "Server-side agent runtime",
      metrics: [
        { label: "Mode", value: agent.mode },
        { label: "Approval ceiling", value: `Class ${agent.approvalClass}` },
        { label: "Evaluation gate", value: `${Math.round(agent.evaluationGateBasisPoints / 100)}%` },
        { label: "Executions", value: String(agent.runtime.executions) },
        { label: "Completed", value: String(agent.runtime.completed) },
        { label: "Mean latency", value: `${agent.runtime.averageLatencyMs} ms` },
        { label: "Cost", value: `${agent.runtime.costMicrounits} µ` },
      ],
      evidence: [
        { label: "Enabled", value: agent.enabled ? "Yes" : "Paused by active configuration", source: agent.mode },
        { label: "Kill switch", value: agent.killSwitchAvailable ? "Available" : "Unavailable", source: "Governance manifest" },
      ],
      steps: [
        { label: "Configure", detail: "Manifest loaded from the server pack", state: "done" },
        { label: "Execute", detail: `${agent.runtime.executions} traced executions`, state: "done" },
        { label: "Govern", detail: `Class ${agent.approvalClass} approval ceiling`, state: "current" },
        { label: "Verify", detail: "Human review before any command", state: "pending" },
      ],
      ...
      <section className="panel agent-message-panel">
        ...
        <div className="agent-message-list">
          {(snapshot?.messages ?? []).map((message) => (
            <button type="button" className="drillable-surface" onClick={() => onOpenDetail({ id: message.id, kind: "Agent message", title: message.summary, summary: `${message.kind} handoff from ${message.from} to ${message.to}.`, status: message.status, tone: message.status === "blocked" ? "red" : message.status === "review" ? "amber" : "mint", owner: message.from, scope: message.to, metrics: [{ label: "Kind", value: message.kind }, { label: "Recorded", value: new Date(message.time).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }) }], evidence: [{ label: "From", value: message.from }, { label: "To", value: message.to }, { label: "Status", value: message.status }], steps: [{ label: "Emit", detail: "Message produced on the server fabric", state: "done" }, { label: "Route", detail: `${message.from} → ${message.to}`, state: "done" }, { label: "Review", detail: message.status, state: message.status === "blocked" ? "blocked" : message.status === "review" ? "current" : "done" }], primary: { label: "Open Swarm Control", target: "ecosystem" } })} key={message.id}>

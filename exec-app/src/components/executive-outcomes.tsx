...
export default function ExecutiveOutcomes({ onNavigate, onOpenDetail }: { onNavigate: (id: NavTarget) => void; onOpenDetail: OpenWorkflowDetail }) {
    ...
    <div className="view-stack executive-view">
      ...
      <section className="executive-outcome-grid">
        {outcomeCards.map((outcome) => {
          ...
          const target: PlatformNavId = outcome.id === "regulatory" ? "cms" : outcome.id === "operational" ? "facility" : outcome.id === "clinical" ? "command" : "ecosystem";
      ...
      <section className="executive-main-grid">
        ...
        <aside className="executive-side-stack">
          <button className="panel enterprise-score drillable-surface" type="button" onClick={() => openExecutiveDetail("Balanced outcome index", "Weighted composite from the active enterprise strategy pack.", "92.4 · +1.8", "platform", [{ label: "Composite", value: "92.4", source: "Active enterprise strategy pack" }, { label: "Change", value: "+1.8", source: period }])}><span className="score-orbit"><Target size={23} /></span><div><Eyebrow>Balanced outcome index</Eyebrow><strong>92.4</strong><p>Weighted by the active enterprise strategy pack.</p></div><Tag tone="mint">+1.8</Tag></button>
          <article className="panel outcome-dependencies"><div className="panel-title-row"><div><Eyebrow>Outcome dependencies</Eyebrow><h2>What moved the result</h2></div><Network size={17} /></div>{[{label:"Care continuity",value:97,detail:"Transitions + transport + capacity",target:"command" as PlatformNavId},{label:"Clinical quality",value:93,detail:"Access + assessment + quality",target:"command" as PlatformNavId},{label:"Revenue integrity",value:89,detail:"Treatment + eligibility + claims",target:"ecosystem" as PlatformNavId},{label:"Regulatory readiness",value:94,detail:"Measures + sources + reconciliation",target:"cms" as PlatformNavId}].map((item) => <button className="dependency-row drillable-surface" type="button" onClick={() => openExecutiveDetail(item.label, item.detail, `${item.value}%`, item.target, [{ label: "Dependency score", value: `${item.value}%`, source: item.detail }])} key={item.label}><div><strong>{item.label}</strong><small>{item.detail}</small><span>{item.value}%</span></div><ProgressBar value={item.value} tone={item.value < 90 ? "amber" : "mint"} /></button>)}</article>
        </aside>
      </section>
      ...
      <section className="executive-lower-grid">
        <article className="panel value-portfolio">
          ...
          <div className="value-flow">{[{stage:"Signals",value:String(runtime?.counts.events ?? "—"),detail:"persisted events",target:"ecosystem" as PlatformNavId},{stage:"Insights",value:String(runtime?.counts.insights ?? "—"),detail:"cross-domain",target:"intelligence" as PlatformNavId},{stage:"NBAs",value:String(runtime?.counts.actions ?? "—"),detail:"human review",target:"ecosystem" as PlatformNavId},{stage:"Commands",value:String(runtime?.counts.commands ?? "—"),detail:"authorized",target:"command" as PlatformNavId},{stage:"Outcomes",value:String(runtime?.counts.acknowledgements ?? "—"),detail:"event verified",target:"command" as PlatformNavId}].map((item, index) => <button type="button" className="drillable-surface" onClick={() => openExecutiveDetail(item.stage, `${item.value} ${item.detail} in the active value pipeline.`, item.detail, item.target)} key={item.stage}><span>{index + 1}</span><strong>{item.value}</strong><small>{item.stage} · {item.detail}</small>{index < 4 ? <ArrowRight size={14} /> : null}</button>)}</div>
        ...
        <article className="panel operating-model-card">
          ...
          <button className="button button-secondary" onClick={() => onNavigate("platform")} type="button">Inspect operating model <ArrowRight size={14} /></button>
        </article>
      </section>

...
use crate::cell::{CfcCell, ContinuousTimeCell};
use crate::config::CfcConfig;
use crate::mapping::FeatureMap;

pub struct CfcOutput {
...
/// Sequence-level CfC model (liquid-ai.md §13, §47): a [`CfcCell`] plus an optional output
/// projection and output feature mapping.
pub struct Cfc {
    cell: CfcCell,
...
impl Cfc {
    pub fn new(config: CfcConfig, vb: VarBuilder) -> Result<Self> {
        ...
        let cell = CfcCell::new(config, vb.pp("cell"))?;
    ...
    pub fn cell(&self) -> &CfcCell {
        &self.cell
    }

    /// Single-step streaming inference (liquid-ai.md §44): fetch hidden state, compute
    /// `delta_t`, single cell step, persist the new hidden state — no sequence tensor
    ...
    /// `inputs`: `[batch, time, input_size]`. `delta_t`: `[batch, time, 1]`, or `None` for a
    /// uniform `Δt=1.0` (liquid-ai.md §13).
    pub fn forward(
        &self,
        inputs: &Tensor,
        initial_state: Option<&Tensor>,
        delta_t: Option<&Tensor>,
    ) -> Result<CfcOutput> {

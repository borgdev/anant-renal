...
export class BullMQJobBus implements JobBus {
  ...
  constructor(opts: BullMQJobBusOptions) {
    // BullMQ queue names allow only [A-Za-z0-9_-] — a colon is rejected with
    // "Queue name cannot contain :". Use hyphens, not `:`.
  ...
  async enqueue<T>(kind: string, payload: T, opts: EnqueueOptions): Promise<JobHandle> {
    // BullMQ custom job ids cannot contain `:` (e.g. event ids are `event:…`),
    // so sanitize the composite id — this still preserves per-kind dedup.
    const jobId = `${kind}:${opts.idempotencyKey}`.replace(/:/g, '_');

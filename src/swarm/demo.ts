...
export function buildSwarmDemo(now: () => string = NOW): SwarmDemoState {
  ...
  const nbas = rankNextBestActions(attachInsightBelief([
    { title: 'Protect 43 weekend treatments', cells: ['continuity', 'workforce'], scopeType: 'region', subject: 'region:middle-tennessee', owner: 'ROD', due: '2 hours', evidence: Array.from({ length: 18 }, (_, i) => ev(`ev:${i}`, 'object')), consensus: 0.96, approvalClass: 'B', expectedOutcome: 43, urgency: 0.95, policyCost: 0.4, risk: 0.2, insightKind: 'coverage.proposal' },
    { title: 'Rebalance 24.5 chair-hours', cells: ['access', 'growth'], scopeType: 'facility', subject: 'facility:riverbend-franklin', owner: 'ROD', due: 'Today', evidence: Array.from({ length: 14 }, (_, i) => ev(`ev:${i}`, 'object')), consensus: 0.93, approvalClass: 'B', expectedOutcome: 24.5, urgency: 0.8, policyCost: 0.3, risk: 0.15, insightKind: 'capacity.proposal' },
    { title: 'Open catheter reduction review', cells: ['clinical quality'], scopeType: 'market', subject: 'market:nashville-south', owner: 'Quality Director', due: 'This week', evidence: Array.from({ length: 22 }, (_, i) => ev(`ev:${i}`, 'object')), consensus: 0.94, approvalClass: 'C', expectedOutcome: 31, urgency: 0.6, policyCost: 0.5, risk: 0.3, insightKind: 'assessment.insight' },
    { title: 'Resolve clean-claim mapping variance', cells: ['revenue', 'cms'], scopeType: 'division', subject: 'division:southeast', owner: 'VP Finance', due: '48 hours', evidence: Array.from({ length: 28 }, (_, i) => ev(`ev:${i}`, 'object')), consensus: 0.99, approvalClass: 'B', expectedOutcome: 184, urgency: 0.7, policyCost: 0.35, risk: 0.1, insightKind: 'claim.evidence' },

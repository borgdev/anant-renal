...
import { greenTeamChecks, traceSpans } from "../../lib/demo-data";
...
export default function AssuranceCenter({ onOpenDetail }: { onOpenDetail: OpenWorkflowDetail }) {
  ...
  useEffect(() => {
    let active = true;
    void ensureRuntime().then((snapshot) => { if (active) setRuntime(snapshot); }).catch((error) => { if (active) setRuntimeError(error instanceof Error ? error.message : "Runtime assurance unavailable"); });
    return () => { active = false; };
  }, []);
  ...
  async function runReplay() {
    ...
    try {
      const response = await fetch("/api/harness", {
        ...
        body: JSON.stringify({
          kind: "red-team-replay",
          runId: crypto.randomUUID(),
          scenarioId: scenario.id,
        }),
      });
      const payload = await response.json() as { result?: typeof redResult; error?: string };
      if (!response.ok || !payload.result) throw new Error(payload.error ?? "Evaluation ledger rejected the replay");
  ...
  function openAssuranceDetail(title: string, summary: string, status: string, evidence: Array<{ label: string; value: string; source?: string }> = [], target: NavigationId = "configuration") {
    ...
    <div className="view-stack assurance-view">
      <header className="view-heading">
        <div>
          ...
          <h1>No intelligence advances on trust alone.</h1>
